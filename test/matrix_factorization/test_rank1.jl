...
fm = fmTrain(sparse(X), y, dim = dim, alpha = 0.3)
@test fm.w0 == 0
@test fm.w == zeros(num_factors)
@test_approx_eq_eps 0.0 fmEvaluateRMSE(fm, sparse(X), y) 1e-7
...
dim = (0, 0, 1)
info("Decomposing Rank 1 Matrix")
fm = fmTrain(sparse(X), y, dim = dim, task = :classification, alpha = 0.3)
@test fm.w0 == 0
@test fm.w == zeros(num_factors)
